...
The sample size after omitting the missing values, due to the vast amount of missing, was 6691. The average age of the respondents was 44.39 with a standard deviation of 26.20. The participants were 89.8\% female, 77.7\% white, 10.5\% Black, 7.90\% Asian, and 3.48\% reported as residual. This sample size has some significant bias around sex and race. There answer options for the demographics were very binary and lacking in self-description options.

\subsection{Measures}\label{measures}

\subsubsection{Childhood well-being scale}\label{childhood-well-being-scale}

The childhood well-being scale was created using select objects from the SIPP section on child well-being. For this report there were only four true or false questions used to create the cwb\_scale which stands for the childhood well-being scale. This was also reported data from the parents about children at a variety of ages. The Cronbach's alpha for this scale was 0.04. This is a very low alpha, probably due to the few number of items and the randomness in selection of the items.
...
\caption{\label{tab:descriptive table and plots}Descriptivse for Childhood Wellbeing Scale Items}

\begin{tabular}{lll}
\toprule
Item & \multicolumn{1}{c}{Mean} & \multicolumn{1}{c}{SD}\\
\midrule
CWB Item 1 & 1.61 & 0.49\\
CWB Item 2 & 1.91 & 0.29\\
CWB Item 3 & 1.95 & 0.22\\
CWB Item 4 & 1.49 & 0.50\\
\bottomrule
\addlinespace
\end{tabular}
...
\begin{verbatim}
## Warning: Removed 4275 rows containing non-finite outside the scale range
## (`stat_bin()`).
\end{verbatim}

\includegraphics{Residential_Instability_Report_files/figure-latex/descriptive table and plots-1.pdf}

The p-value when comparing residential stability marker 1 and the cwb\_scale was very small, c(t = 11.479617716626), c(df = 4205), 4.66323062075316e-30, c(0.480659060278834, 0.678649781564255), c(\texttt{mean\ of\ x} = 1.89215442092154, \texttt{mean\ of\ y} = 1.3125), c(\texttt{difference\ in\ means} = 0), 0.0504942268314413, two.sided, Two Sample t-test, dat\_sim\(res_1_sim and dat_sim\)cwb\_scale\_sim.
\newpage
